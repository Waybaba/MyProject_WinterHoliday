...
with tf.name_scope('variables_all'):
    A1 = tf.Variable(tf.random_normal(shape=[3, hidden_layer_nodes]),name='muli_1')  # inputs -> hidden nodes
    b1 = tf.Variable(tf.random_normal(shape=[hidden_layer_nodes]),name='bias_1')  # one biases for each hidden node
    A2 = tf.Variable(tf.random_normal(shape=[hidden_layer_nodes, 1]),name='muti_2')  # hidden inputs -> 1 output
    b2 = tf.Variable(tf.random_normal(shape=[1]),name='bias_2')   # 1 bias for the output
