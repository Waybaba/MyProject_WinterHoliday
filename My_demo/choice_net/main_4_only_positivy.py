...
class ModelPredict20:
    ...
    def random_choose(self,data_50,choose_num=20):
        data_20 = np.empty(shape=(data_50.shape[0], 20, 75))
        mask = np.empty(shape=(data_50.shape[0],50))
        for date_count in range(data_50.shape[0]):
            current_date = data_50[date_count]
            index = np.random.choice(a=np.arange(50), size=20, replace=False)
            index = np.sort(index, axis=-1)  # 对第1维升序排序
            index_01 = np.zeros(shape=(50))
            index_01[index] = 1.0
            data_20[date_count] = current_date[index]
            mask[date_count] =  index_01
        return data_20,mask
...
def modify_mask(mask,right_flag):
    ...
    return mask
def get_positive(data,mask,right_flag):
    index_list = []
    for index in range(data.shape[0]):
        if right_flag[index]==1.0:
            index_list.append(index)
    return data[index_list],mask[index_list]


choose_train_once_num = 500
choose_train_times = 200
main_verbose = False
load_name = "test"
save_name = "test_simple_net_dense_positiveonly"
...
if __name__ == "__main__":
    ...
    for round_count in range(round_of_dule):
        ...
        for i in range(choose_train_times):
            ...
            chose_data_20, chose_mask_now = model_predict_20.random_choose(data_50=chose_data)
            ...
            chose_data_posi,chose_mask_mod_posi = get_positive(data=chose_data,mask=chose_mask_now,right_flag=chose_right_flag)
            model_choose.train(data=chose_data_posi,label=chose_mask_mod_posi,epotch=3) # train
...
# x = layers.Dense(128,activation='relu')(inputs_data)
#         x = layers.Dense(128, activation='relu')(x)
#         x = layers.Dense(50,activation='relu')(x)
#         x = layers.Dense(50, activation='relu')(x)
#         predictions = layers.Dense(50, activation='sigmoid')(x)